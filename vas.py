...
def get_args():
    ...
    parser.add_argument('--screen', type=int, choices=[0,1], default=0, help='local (0) or remote (1)')
...
def init_objects(x, y, screen):
    ...
    fullscr=False
    if screen: fullscr=True
    win = visual.Window(size=(x,y), pos=(0,0), color='white', allowGUI=True, fullscr=fullscr, screen=screen)
    title = visual.TextStim(win=win, text=None, color='black', pos=[0,.25])
    time = visual.TextStim(win=win, text='', color='red', pos=[0,-.25])
    ...
    rect.fillColor = 'red'
    pain0 = visual.TextStim(win=win, color='black', height=.055, pos=[-.7, 0])
    pain10 = visual.TextStim(win=win, color='black', height=.055, pos=[.75, 0])
...
def main():
    ...
    kb, timer, win, title, time, outline, rect, pain0, pain10 = init_objects(x, y, args.screen)
    ...
    for pain_type in ['PAIN INTENSITY', 'PAIN UNPLEASANTNESS']:
        ...
        while timer.getTime() > 0:
            ...
            if remaining < 3: time.text = f'{remaining:.1f}'
        ...
        print(Style.bold+Style.red + f'{pain_type[5:8]}:\t{pain_rating:.2f}' + Style.end)
        win.flip()
        core.wait(1)
